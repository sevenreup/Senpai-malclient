package com.skybox.seven.customviews.indicators
...
class FancyIndicator @JvmOverloads constructor(
    ...
    init {
        ...
        if (attrs != null) {
            val a = getContext().obtainStyledAttributes(attrs, R.styleable.DefaultIndicator)

            // Dots attributes
            dotIndicatorColor =
                a.getColor(R.styleable.DefaultIndicator_dotsColor, dotIndicatorColor)
            dotsStrokeColor =
                a.getColor(R.styleable.DefaultIndicator_dotsStrokeColor, dotIndicatorColor)

            // Spring dots attributes
            dotsStrokeWidth = a.getDimension(
                R.styleable.DefaultIndicator_dotsStrokeWidth,
    ...
    private fun buildDot(stroke: Boolean): ViewGroup {
        val dot = LayoutInflater.from(context).inflate(
            R.layout.layout_fancy_indicator, this,
            false
        ) as ViewGroup
